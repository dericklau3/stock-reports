...
正式产业链雷达必须先保存 markdown 文档，再在最终回复里给用户文件路径和摘要。保存文档按此顺序：
...
- **只回答不保存文档**：正式产业链雷达必须保存 markdown 到 `research/industry-chain-radar/`，最终回复给出链接。
...
- [ ] 正式产业链雷达已保存 markdown 到 `research/industry-chain-radar/`。
- [ ] 高优先级环节都有完整台账，或已明确降级。
